...
6146598194 = os.getenv('6146598194', '6146598194') # သင့် ID ဂဏန်းအစစ် ပြောင်းထည့်ပါ
...
@app.route('/')
def home():
    return "Bot is running 24/7 with Payment & 20 Tables!"
...
def main_menu_keyboard():
    ...
    markup.add(
        KeyboardButton('💳 ငွေပေးချေရန်'),  # Payment ခလုတ်အသစ်
...
def handle_text(message):
    ...
    # Menu (မီနူး အသစ်များ ထပ်တိုးထားသည်)
        ...
        markup.add(
            ...
            InlineKeyboardButton("🦞 ပင်လယ်စာအမျိုးမျိုး", callback_data="cat_seafood"), # အသစ်
            InlineKeyboardButton("🍢 အကင်စုံ (BBQ)", callback_data="cat_bbq"), # အသစ်
    ...
    # Booking (ဝိုင်း ၂၀ အထိ ရွေးချယ်ရန် ပြင်ဆင်ထားသည်)
    elif text == '📅 စားပွဲကြိုတင်မှာယူရန်':
        markup = InlineKeyboardMarkup(row_width=4)
        tables = []
        for i in range(1, 21): # ဝိုင်း ၁ မှ ၂၀ အထိ
    ...
    # Delivery (ပိတ်သိမ်းထားကြောင်း ပြင်ဆင်ထားသည်)
    elif text == '🚚 ပို့ဆောင်မှု (Delivery)':
        reply_msg = (
            "🚚 **Delivery ဝန်ဆောင်မှု**\n\n"
            "⚠️ **အခုရက်အတွင်းမှာ ပို့ဆောင်မှု (Delivery) ဝန်ဆောင်မှုကို ယာယီ ပိတ်သိမ်းထားပါပြီခင်ဗျာ။**\n\n"
            "ဆိုင်သို့ ကိုယ်တိုင်လာရောက် အားပေးရန် နွေးထွေးစွာ ဖိတ်ခေါ်အပ်ပါသည်။ အဆင်မပြေမှုများရှိခဲ့ပါက တောင်းပန်အပ်ပါသည်။"
        )
        bot.send_message(chat_id, reply_msg, parse_mode='Markdown')

    # FAQ (မွေးနေ့ပွဲ ဖြေကြားချက်နှင့် မေးခွန်းသစ်များ)
        ...
        reply_msg = (
            ...
            "A: ရပါတယ်။ ငွေသားအပြင် မိုဘိုင်းပေးချေမှုအမျိုးမျိုးကို % အပိုပေးစရာမလိုဘဲ လက်ခံပါသည်။\n\n"
            "**Q: ပါတီပွဲ / မွေးနေ့ပွဲများအတွက် နေရာငှားလို့ရလား?**\n"
            "A: ရပါပြီခင်ဗျာ။ လူ ၃၀ ကနေ ၅၀ အထိ ဆံ့တဲ့ ဆိုင်နေရာ ကျယ်ကျယ်ဝန်းဝန်း ရှိပါတယ်။ သီးသန့်ခန်းတော့ မဟုတ်ပါဘူးခင်ဗျာ။ ကြိုတင် Booking ပေးဖို့တော့ လိုပါတယ်။\n\n"
            "**Q: အိမ်မွေးတိရစ္ဆာန် (Pet) ခေါ်လာလို့ရပါသလား?**\n"
            "A: တောင်းပန်ပါတယ်ခင်ဗျာ၊ အခြားစားသုံးသူများ အဆင်ပြေစေရန် အိမ်မွေးတိရစ္ဆာန် ခွင့်မပြုထားပါဘူးခင်ဗျာ။"
        )
        bot.send_message(chat_id, reply_msg, parse_mode='Markdown')
    ...
    # ငွေပေးချေရန် (အသစ်)
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("KPay ဖြင့် ပေးချေမည်", callback_data="pay_kpay"),
            InlineKeyboardButton("WavePay ဖြင့် ပေးချေမည်", callback_data="pay_wave"),
            InlineKeyboardButton("CB Pay ဖြင့် ပေးချေမည်", callback_data="pay_cb"),
            InlineKeyboardButton("AYA Pay ဖြင့် ပေးချေမည်", callback_data="pay_aya")
        )
        bot.send_message(
            chat_id, 
            "💳 **ငွေပေးချေရန် ရွေးချယ်ပါ**\n\nလူကြီးမင်း အသုံးပြုလိုသော ငွေပေးချေမှုစနစ်ကို အောက်ပါခလုတ်များမှ ရွေးချယ်ပါ-", 
            reply_markup=markup,
            parse_mode='Markdown'
        )
...
# ---------------------------------------------------------
# Payment Callbacks & Slip Handling (အသစ်)
# ---------------------------------------------------------
@bot.callback_query_handler(func=lambda call: call.data.startswith('pay_'))
def callback_payment(call):
    chat_id = call.message.chat.id
    
    # ဤနေရာတွင် မိမိဆိုင်၏ အကောင့်အမည်၊ ဖုန်းနံပါတ်နှင့် QR Code ပုံ (URL) များကို ပြင်ထည့်ပါ။
    if call.data == "pay_kpay":
        acc_name = "yee yee cho (A Thae Swae)"
        phone = "09250597667"
        qr_url = "https://cdn.phototourl.com/member/2026-09-25-000873aa-a533-48c4-9723-22ffb4bdc5ec.jpg" # သင်၏ KPay QR ပုံ Link ထည့်ပါ
        pay_type = "KPay"
    elif call.data == "pay_wave":
        acc_name = "yee yee cho (A Thae Swae)"
        phone = "09675494412"
        qr_url = "https://cdn.phototourl.com/member/2026-09-25-c3c4312c-1261-46a7-9341-7a42072afeb7.jpg" # သင်၏ Wave QR ပုံ Link ထည့်ပါ
        pay_type = "WavePay"
    elif call.data == "pay_cb":
        acc_name = "yee yee cho"
        phone = "0000 1111 2222 3333"
        qr_url = "https://images.unsplash.com/photo-1605810230434-7631ac76ec81?auto=format&fit=crop&w=400&q=60" # သင်၏ CB Pay QR ပုံ Link ထည့်ပါ
        pay_type = "CB Pay"
    elif call.data == "pay_aya":
        acc_name = "pyae phyo aung"
        phone = "09977466809"
        qr_url = "https://cdn.phototourl.com/member/2026-09-25-43ae1131-867f-4b6c-8662-9f264704ade3.jpg" # သင်၏ AYA Pay QR ပုံ Link ထည့်ပါ
        pay_type = "AYA Pay"

    caption_text = (
        f"💳 **{pay_type} ဖြင့် ငွေပေးချေရန်**\n\n"
        f"👤 အကောင့်အမည်: **{acc_name}**\n"
        f"📱 နံပါတ်: `{phone}` (Copy ကူးရန် နံပါတ်ကိုနှိပ်ပါ)\n\n"
        "အထက်ပါ QR Code ကို Scan ဖတ်၍ဖြစ်စေ၊ ဖုန်းနံပါတ်သို့ဖြစ်စေ ငွေလွှဲပေးချေနိုင်ပါသည်။"
    )
    
    bot.send_photo(chat_id, qr_url, caption=caption_text, parse_mode='Markdown')
    
    msg = bot.send_message(
        chat_id, 
        "📤 ငွေလွှဲပြီးပါက **ငွေလွှဲပြေစာ (Payment Slip) Screenshot** ကို ယခု Chat Box သို့ ပေးပို့ပေးပါခင်ဗျာ။", 
        reply_markup=cancel_keyboard(),
        parse_mode='Markdown'
    )
...
def process_payment_slip(message, pay_type):
    if message.text == '❌ ပယ်ဖျက်မည်':
        bot.send_message(message.chat.id, "ငွေပေးချေမှု လုပ်ငန်းစဥ်ကို ပယ်ဖျက်လိုက်ပါပြီ။", reply_markup=main_menu_keyboard())
        return

    # User က ပုံ(Photo) ပို့သလား စစ်ဆေးခြင်း
    if message.photo:
        user = message.from_user
        username = f"@{user.username}" if user.username else "မရှိပါ"
        
        admin_caption = (
            f"💰 **ငွေလွှဲပြေစာ (Slip) အသစ်ဝင်လာပါပြီ** 💰\n\n"
            f"👤 Customer: {user.first_name}\n"
            f"🔗 Telegram: {username}\n"
            f"💳 ငွေပေးချေသည့်စနစ်: **{pay_type}**"
        )
        
        try:
            # အကြီးဆုံး resolution ရှိတဲ့ပုံကို ယူမယ်
            file_id = message.photo[-1].file_id
            bot.send_photo(ADMIN_CHAT_ID, file_id, caption=admin_caption, parse_mode='Markdown')
            bot.reply_to(message, "✅ ငွေလွှဲပြေစာ လက်ခံရရှိပါပြီ။ အော်ဒါအတွက် ကျေးဇူးအထူးတင်ရှိပါတယ်ခင်ဗျာ။", reply_markup=main_menu_keyboard())
        except Exception as e:
            logging.error(f"Slip Forward Error: {e}")
            bot.reply_to(message, "⚠️ Admin ထံသို့ ပို့ဆောင်ရာတွင် အဆင်မပြေဖြစ်သွားပါသည်။ ဖုန်းဖြင့် ဆက်သွယ်ပေးပါ။", reply_markup=main_menu_keyboard())
    else:
        # စာရိုက်ပို့လိုက်ရင် ပုံပို့ဖို့ ပြန်တောင်းမယ်
        msg = bot.reply_to(message, "⚠️ ကျေးဇူးပြု၍ ငွေလွှဲပြေစာ (Screenshot) **ပုံ(Photo)** ကိုသာ ပေးပို့ပေးပါခင်ဗျာ။", reply_markup=cancel_keyboard())
        bot.register_next_step_handler(msg, process_payment_slip, pay_type)

# ---------------------------------------------------------
# Menu Categories Callbacks (Menu အသစ်များ ပါဝင်သည်)
# ---------------------------------------------------------
...
def callback_menu_categories(call):
    ...
    if call.data == "cat_drinks":
        ...
        desc = "🦞 **ပင်လယ်စာအမျိုးမျိုး**\n- ပုဇွန်ကင်\n- ကင်းမွန်သုတ်\n- ဂဏန်းမဆလာ"
    elif call.data == "cat_bbq":
...
# ---------------------------------------------------------
# Table Booking Callback (ဝိုင်း ၂၀ အတွက်)
# ---------------------------------------------------------
...
def callback_book_table(call):
    ...
    bot.register_next_step_handler(msg, process_booking_step, table_no)
    bot.answer_callback_query(call.id)

def process_booking_step(message, table_no):
    if message.text == '❌ ပယ်ဖျက်မည်':
...
def handle_order_status(call):
    ...
    if action == "accept":
        bot.send_message(customer_id, "🎉 **သတင်းကောင်းပါ!**\nလူကြီးမင်း၏ အော်ဒါကို ဆိုင်မှ လက်ခံလိုက်ပါပြီ။")
